import { supabase, success, errorResponse, generateId, sha256, passwordHash, nowISO, appDate, appTime, timeToMinutes, minutesBetween, haversineDistance, requireAuth, requireRole, getManagedProjectIds, canManageProject, getCurrentAssignment, getCurrentEmployeeShift, writeAuditLog } from "./core";
...
export async function updateEmployee(session: SessionContext, body: Record<string, unknown>) {
  ...
  await writeAuditLog(session.user.user_id,'update_employee','employees',employeeId,{changes}); return success(data);
}
