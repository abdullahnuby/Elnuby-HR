import { supabase, success, errorResponse, generateId, nowISO, writeAuditLog } from "./core";
...
export async function updateShift(session: SessionContext, body: Record<string, unknown>) {
  ...
  if(error) return errorResponse(error.message,500); if(!data) return errorResponse('الوردية غير موجودة',404); await writeAuditLog(session.user.user_id,'update_shift','shifts',shiftId,{changes}); return success(data);
}
