...
export default function Home(){
 ...
 {section==='employees'&&<Employees employees={employees} projects={projects} shifts={shifts} employeeForm={employeeForm} setEmployeeForm={setEmployeeForm} createEmployee={createEmployee} selectedEmployee={selectedEmployee} setSelectedEmployee={setSelectedEmployee} selectedProject={selectedProject} setSelectedProject={setSelectedProject} selectedShift={selectedShift} setSelectedShift={setSelectedShift} assignProject={assignProject} busy={busy} />}
 {section==='shifts'&&<Shifts shifts={shifts} rows={rows} shiftForm={shiftForm} setShiftForm={setShiftForm} createShift={createShift} busy={busy} />}
 {section==='projects'&&<Projects projects={projects} employees={employees} projectForm={projectForm} setProjectForm={setProjectForm} createProject={createProject} busy={busy} />}
...
function Employees({employees,projects,shifts,employeeForm,setEmployeeForm,createEmployee,selectedEmployee,setSelectedEmployee,selectedProject,setSelectedProject,selectedShift,setSelectedShift,assignProject,busy}:{employees:Employee[];projects:Project[];shifts:Shift[];employeeForm:any;setEmployeeForm:any;createEmployee:()=>void;selectedEmployee:string;setSelectedEmployee:any;selectedProject:string;setSelectedProject:any;selectedShift:string;setSelectedShift:any;assignProject:()=>void;busy:boolean}){
 return <section className="panel page-panel">
  ...
  <div className="request-card"><h3>إضافة موظف جديد</h3><p>سيتم إنشاء ملف الموظف وتعيينه على المشروع والوردية المختارين في نفس العملية.</p>
  ...
  <div className="request-card"><h3>نقل موظف + تغيير الوردية</h3><div className="formgrid"><select value={selectedEmployee} onChange={e=>setSelectedEmployee(e.target.value)}><option value="">اختر الموظف</option>{employees.map(e=><option key={e.employee_id} value={e.employee_id}>{e.name} — {e.employee_id}</option>)}</select><select value={selectedProject} onChange={e=>setSelectedProject(e.target.value)}><option value="">اختر المشروع</option>{projects.map(p=><option key={p.project_id} value={p.project_id}>{p.name}</option>)}</select><select value={selectedShift} onChange={e=>setSelectedShift(e.target.value)}><option value="">اختر الوردية</option>{shifts.map(s=><option key={s.shift_id} value={s.shift_id}>{s.name} — {s.start_time}</option>)}</select></div><button className="secondary" disabled={busy||!selectedEmployee||!selectedProject||!selectedShift} onClick={assignProject}>حفظ المشروع والوردية</button></div>
