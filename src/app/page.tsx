...
type Project={project_id:string;name:string;location?:string;status?:string;project_manager_id?:string};
...
function Employees({employees}:{employees:Employee[]}){return <section className="panel page-panel"><div className="panel-head"><div><h2>الموظفون</h2><p>قاعدة بيانات الموظفين النشطين والحاليين.</p></div><span className="count-pill">{employees.length} موظف</span></div><div className="search-row"><input placeholder="بحث بالاسم أو الرقم الوظيفي…" onChange={()=>{}}/></div><Table headers={['الرقم','الاسم','الوظيفة','القسم','الحالة']} rows={employees.map(e=>[e.employee_id,e.name,e.job_title||'—',e.department||'—',e.status||'ACTIVE'])}/></section>}
function Projects({projects}:{projects:Project[]}){return <section className="panel page-panel"><div className="panel-head"><div><h2>المشاريع</h2><p>المشروعات التي لديك صلاحية الوصول إليها.</p></div><span className="count-pill">{projects.length} مشروع</span></div><div className="project-cards">{projects.map(p=><div className="project-card" key={p.project_id}><div className="project-icon">▦</div><div><h3>{p.name||p.project_id}</h3><p>{p.location||'الموقع غير محدد'}</p><span>{p.status||'ACTIVE'}</span></div></div>)}</div>{!projects.length&&<Empty text="لا توجد مشاريع مسجلة حتى الآن."/>}</section>}
