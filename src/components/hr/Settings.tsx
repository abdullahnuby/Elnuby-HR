...
export default function Settings({role='HR_MANAGER'}:{role?:string}){
 ...
 return <section className="panel page-panel hr-settings">
  <div className="hr-settings-hero"><div><div className="eyebrow">مركز تحكم ELNUBY HR</div><h2>إعدادات النظام</h2><p>قواعد التشغيل ولوائح الإجازات ومركز إكسل في مكان واحد.</p></div><div className="hr-system-status"><span className="status-dot"/> النظام متصل</div></div>
  <div className="hr-settings-tabs">
   {[
    ['overview','نظرة عامة','dashboard'],['attendance','الحضور والورديات','attendance'],
    ...(canEdit?[['leaves','لوائح الإجازات','leaves'],['excel','استيراد وتصدير إكسل','reports']]:[]),
  ...
  {tab==='overview'&&<div className="hr-settings-body">
   <div className="hr-stat-grid"><div><span>التوقيت</span><strong>توقيت مصر</strong><small>يُدار من إعدادات النظام</small></div><div><span>الحضور</span><strong>تحديد الموقع والنطاق</strong><small>تحقق آمن من الخادم</small></div><div><span>الإجازات</span><strong>محرك لوائح الإجازات</strong><small>حسب نوع الإقامة واللائحة</small></div><div><span>Excel</span><strong>استيراد وتصدير</strong><small>فحص قبل الحفظ + سجل مراجعة</small></div></div>
   <div className="hr-settings-columns"><section><div className="hr-section-title"><span>01</span><div><h3>قواعد الإجازات</h3><p>الرصيد لا يعتمد على رقم ثابت داخل الواجهة.</p></div></div>{[['مغترب','7 أيام لكل 35 يوم حسب اللائحة الحالية.'],['مقيم','21 يوم سنويًا حسب اللائحة الحالية.'],['مرضية','لا تستهلك الرصيد السنوي وتحتاج مستندًا طبيًا.'],['التغيير','إدارة الموارد البشرية تستطيع إصدار لائحة جديدة بتاريخ سريان دون كسر التاريخ السابق.']].map(([a,b])=><div className="hr-rule" key={a}><b>{a}</b><span>{b}</span></div>)}</section><aside><div className="hr-section-title"><span>02</span><div><h3>حوكمة البيانات</h3><p>كل تغيير إداري مهم قابل للمراجعة.</p></div></div>{[['المستندات الطبية','Bucket خاص + روابط مؤقتة'],['Excel','Validation قبل Commit'],['الرصيد','حساب قابل للتتبع'],['التدقيق','Audit لكل عملية إدارة']].map(([a,b])=><div className="hr-source" key={a}><span>{a}</span><b>{b}</b></div>)}</aside></div>
  </div>}
  {tab==='attendance'&&<div className="hr-settings-body"><div className="hr-feature"><div className="feature-icon blue"><Icon name="attendance" /></div><div><h3>محرك الحضور والانصراف</h3><p>يتم التحقق من الوقت والموقع والوردية من الخادم، مع دعم الورديات التي تعبر منتصف الليل.</p></div></div><div className="hr-rule-grid">{[['فتح الحضور','من وقت فتح الحضور حتى وقت إغلاق الحضور'],['التأخير','يُحسب من بداية الوردية وزمن التأخير'],['الانصراف','من وقت فتح الانصراف حتى وقت الإغلاق'],['الإغلاق التلقائي','موعد الإغلاق التلقائي'],['الموقع','تحديد الموقع + نطاق الحضور'],['التوقيت','توقيت مصر']].map(([a,b])=><div className="hr-rule-card" key={a}><b>{a}</b><span>{b}</span></div>)}</div></div>}
