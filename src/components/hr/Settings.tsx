...
export default function Settings({role='HR_MANAGER'}:{role?:string}){
 ...
 return <section className="panel page-panel hr-settings">
  ...
  {tab==='overview'&&<div className="hr-settings-body">
   <div className="hr-stat-grid"><div><span>التوقيت</span><strong>توقيت القاهرة (مصر)</strong><small>قابل للتهيئة من إعدادات توقيت النظام</small></div><div><span>الحضور</span><strong>الموقع الجغرافي + نطاق الموقع</strong><small>يتم التحقق من الخادم</small></div><div><span>الإجازات</span><strong>محرك لوائح الإجازات</strong><small>حسب نوع الإقامة واللائحة</small></div><div><span>إكسل</span><strong>استيراد / تصدير</strong><small>فحص قبل الحفظ + سجل تدقيق</small></div></div>
   <div className="hr-settings-columns"><section><div className="hr-section-title"><span>01</span><div><h3>قواعد الإجازات</h3><p>الرصيد لا يعتمد على رقم ثابت داخل الواجهة.</p></div></div>{[['مغترب','7 أيام لكل 35 يوم حسب اللائحة الحالية.'],['مقيم','21 يوم سنويًا حسب اللائحة الحالية.'],['مرضية','لا تستهلك الرصيد السنوي وتحتاج مستندًا طبيًا.'],['التغيير','الموارد البشرية تستطيع إصدار لائحة جديدة بتاريخ سريان دون كسر التاريخ السابق.']].map(([a,b])=><div className="hr-rule" key={a}><b>{a}</b><span>{b}</span></div>)}</section><aside><div className="hr-section-title"><span>02</span><div><h3>حوكمة البيانات</h3><p>كل تغيير إداري مهم قابل للمراجعة.</p></div></div>{[['المستندات الطبية','مجلد خاص + روابط مؤقتة'],['إكسل','فحص قبل الحفظ'],['الرصيد','حساب قابل للتتبع'],['التدقيق','سجل تدقيق لكل عملية إدارة']].map(([a,b])=><div className="hr-source" key={a}><span>{a}</span><b>{b}</b></div>)}</aside></div>
  </div>}
  {tab==='attendance'&&<div className="hr-settings-body"><div className="hr-feature"><div className="feature-icon blue"><Icon name="attendance" /></div><div><h3>محرك الحضور والانصراف</h3><p>الوقت والموقع والوردية يتم التحقق منها في الخادم، مع دعم الورديات التي تعبر منتصف الليل.</p></div></div><div className="hr-rule-grid">{[['فتح الحضور','attendance_open → attendance_close'],['التأخير','start_time + late_minutes'],['الانصراف','checkout_open → checkout_close'],['الإغلاق التلقائي','auto_checkout_time'],['الموقع','الموقع الجغرافي + نطاق الموقع'],['التوقيت','توقيت القاهرة (مصر)']].map(([a,b])=><div className="hr-rule-card" key={a}><b>{a}</b><span>{b}</span></div>)}</div></div>}
