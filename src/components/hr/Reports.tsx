...
export default function Reports({dash,managerDash}:any){
 ...
 return <section className="panel page-panel">
  ...
  {report&&<>
   ...
   <Table headers={['الموظف','حاضر','متأخر','غائب','إجازة','إذن','غير مكتمل','انصراف تلقائي']} rows={(report.employeeSummary||[]).map((e:any)=>[e.name,e.counts?.PRESENT||0,e.counts?.LATE||0,e.counts?.ABSENT||0,e.counts?.LEAVE||0,e.counts?.PERMISSION||0,e.counts?.INCOMPLETE||0,e.counts?.AUTO_CLOSED||0])}/>
   <div className="panel-head" style={{marginTop:24}}><div><h3>السجل اليومي</h3><p>الحالة الفعلية لكل موظف في كل يوم.</p></div></div>
   <Table headers={['التاريخ','الموظف','الحالة','الحضور','الانصراف','التأخير']} rows={(report.rows||[]).map((r:any)=>[r.date,r.employee_name,r.status_label||labels[r.status]||r.status,r.check_in||'—',r.check_out||'—',r.late_minutes?r.late_minutes+' دقيقة':'—'])}/>
  </>}
  {managerDash&&<><div className="panel-head" style={{marginTop:24}}><div><h3>تقرير فريق المشروع</h3><p>الموظفون حسب الحالة اليومية والطلبات المعلقة.</p></div></div><Table headers={['الموظف','المشروع','الحالة','حضور','انصراف']} rows={(managerDash.team||[]).map((e:any)=>[e.name,e.project_name||'—',({PRESENT:'حاضر',CHECKED_IN:'حاضر ولم ينصرف',LATE:'متأخر',ON_LEAVE:'إجازة',ABSENT:'غائب'} as any)[e.state]||e.state,e.attendance?.check_in||'—',e.attendance?.check_out||'—'])}/></>}
 </section>
}
