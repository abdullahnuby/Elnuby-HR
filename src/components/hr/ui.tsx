...
export function Detail({label,value,wide=false,children}:{label:string;value?:ReactNode;wide?:boolean;children?:ReactNode}){const content=value!==undefined?value:children;return <div className={`ux-detail ${wide?'wide':''}`}><span>{label}</span><strong>{content||'—'}</strong></div>}
