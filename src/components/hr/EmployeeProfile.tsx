  ...
  return <div className="profile-drawer" role="dialog" aria-modal="true">
    <div className="profile-drawer-card">
      ...
      <div className="profile-section"><div className="section-title"><div><h3>المسار الوظيفي</h3><p>تاريخ التغييرات والقرارات المرتبطة بالموظف</p></div></div>
        <div className="timeline">{(profile.events || []).length ? profile.events.map((x:any)=><div className="timeline-item" key={x.event_id}><i/><div><strong>{x.title}</strong><span>{x.event_type} — {x.effective_date}</span>{x.description && <p>{x.description}</p>}</div></div>) : <Empty text="لا توجد أحداث وظيفية مسجلة."/>}</div>
